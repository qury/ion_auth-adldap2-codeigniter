...
class Adauth extends CI_Controller {
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $this->load->view('adauth/index', $this->data);
    ...
    public function login()
    {
        ...
        if ($this->ion_auth->logged_in())
        {
            ...
            log_message('debug', 'Adauth::login - You are already logged in.');
        ...
        if ($this->form_validation->run() === true)
        {
            ...
            log_message('debug', 'Adauth::login - Form is submitted succesfully so we are beginning the login process.');
            ...
            {
                log_message('debug', PHP_EOL . 'Succesful local login' . PHP_EOL);
            ...
            {
                log_message('debug', PHP_EOL . 'trying adauth login' . PHP_EOL);
                /*
                ...
                if ($user !== false)
                {
                    log_message('debug', PHP_EOL . 'Succesful adauth login' . PHP_EOL);

                    // Succesful login
                    // Check wheter it is a new user or an existing one.
                    if ($this->exists(strtolower($user['username'])))
                    {
                        log_message('debug', PHP_EOL . 'Succesful adauth login --Existing User' . PHP_EOL);
                    ...
                    {
                        log_message('debug', PHP_EOL . 'Succesful adauth login --New User' . PHP_EOL);
                        // New user
                    ...
                    log_message('debug', 'Adauth::login - redirect to success page: ' . $this->config->item('success', 'adauth'));
                    $this->session->set_flashdata('message', $this->ion_auth->messages());
                ...
                else
                {
                    ...
                    log_message('debug', 'Adauth::login - unsuccesful login, redirect to login page.');
        ...
        else
        {
            ...
            log_message('debug', 'Adauth::login - user is not logging yet, displaying login screen');
    ...
    public function ad_auth(string $user, string $password, string $domain)
    {
        ...
        try
        {
            ...
            // Authentincating
            log_message('debug', 'Adauth::ad_auth - ' . $domain . '\\' . $user);
    ...
    public function pre_set_session(array $user)
    {
        ...
        log_message('debug', 'pre_set_session data: ' . print_r($d, true));
        ...
        log_message('debug', 'pre_set_session actual data: ' . print_r($data, true));
    ...
    public function edit_user(int $id)
    {
        ...
        // Navbar details
        // Navbar values
        foreach (array_keys($this->config->item('navbar')) as $th)
        {
            $this->data['navbar_values'][$th] = $th;
        }
        asort($this->data['navbar_values']);

        $this->data['navbar'] = [
            'name'  => 'navbar',
            'id'    => 'navbar',
            'value' => $this->form_validation->set_value('navbar', $extraData[0]->navbar),
            'list'  => $this->data['navbar_values'],
        ];
